                      ...
                      appUserIds=users.map((user) => user.id).toList();
                        ...
                        return Column(
                          children: [
                                ...
                                itemBuilder: (context,index)=>
                                    InkWell(
                                      child: Stack(
                                        children: [
                                          CircleAvatar(
                                            foregroundImage: NetworkImage(users[appUserIds.indexOf(participantIds[index])].photoUrl!),
                                          ),
                                          const Positioned(
                                            top:2,
                                              left:5,
                            ...
                            ListView(
                              children:(_searchController.text.isEmpty)? usersPresentList(
                                  users, appUserNumber, context,(index){
                                    setState(() {
                                      if(participantIds.contains(users[index].id)) {
                              ...
                              }):SearchMerge(users, appUserNumber,
                                  searchedNumber, searchedUser, context,(index){
                                setState(() {
                                  if(participantIds.contains(users[index].id)) {
                                    participantIds.remove(users[index].id);
                                  }
...
List<Widget> usersPresentList(
    List<Users> users, List<String> appUserNumber, BuildContext context,Function(int) onpress) {
